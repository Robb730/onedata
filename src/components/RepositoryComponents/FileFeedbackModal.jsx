...
export default function FileFeedbackModal({
  ...
  return (
    <ModalPortal>
      <div className="modal-overlay fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-[2px]">
        <div className="w-full max-w-lg h-[82vh] max-h-[680px] flex flex-col bg-white rounded-3xl shadow-[0_40px_100px_rgba(15,23,42,0.28)] border border-slate-200/80 overflow-hidden">
          {/* Header */}
          <div className="relative flex items-start gap-3 px-6 pt-6 pb-5 shrink-0 bg-gradient-to-b from-slate-50/80 to-white border-b border-slate-100">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center shrink-0 shadow-[0_6px_16px_rgba(37,99,235,0.35)]">
              <MessagesSquare
                size={20}
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <h2 className="text-[1rem] font-black text-slate-800 tracking-[-0.01em]">
                Feedback
              </h2>
              <p className="text-[0.75rem] text-slate-400 font-semibold truncate mt-0.5">
                {file.name}
              </p>
            </div>
            <button
          ...
          {/* Privacy notice */}
          <div className="mx-6 mt-4 flex items-center gap-2 rounded-xl bg-blue-50/70 border border-blue-100 px-3.5 py-2.5 shrink-0">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-5 py-4 bg-[radial-gradient(circle_at_1px_1px,theme(colors.slate.100)_1px,transparent_0)] [background-size:18px_18px]">
            {loading ? (
              <div className="flex items-center justify-center h-full gap-2 text-[12px] text-slate-400 font-medium">
                <div className="w-3.5 h-3.5 border-2 border-blue-300 border-t-transparent rounded-full animate-spin" />
                Loading conversation…
              </div>
            ) : messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-white shadow-[0_4px_16px_rgba(15,23,42,0.06)] border border-slate-100 flex items-center justify-center">
                  <MessageSquare
                    size={26}
                ...
                {groups.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`flex flex-col gap-1 max-w-[72%] ${isOwn ? "items-end" : "items-start"}`}
                      >
                        {!isOwn && (
  ...
  return (
    ...
    >
      ...
      <span
        className={`w-12 shrink-0 text-[9px] font-medium text-slate-400 whitespace-nowrap opacity-0 group-hover/message:opacity-100 transition-opacity duration-150 ${
      ...
      >
        {new Date(m.created_at).toLocaleTimeString("en-US", {
          ...
          {/* Composer */}
          <div className="flex items-end gap-2 px-4 py-3.5 border-t border-slate-100 bg-white shrink-0">
            <textarea
              ref={textareaRef}
              value={draft}
              ...
              rows={1}
              className="flex-1 resize-none rounded-2xl border border-slate-200 px-4 py-3 text-[13px] leading-snug placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 transition-all"
            />
            <button
              onClick={handleSend}
              disabled={!draft.trim() || sending}
              className="shrink-0 w-11 h-11 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-slate-200 disabled:to-slate-200 disabled:cursor-not-allowed text-white flex items-center justify-center transition-all shadow-[0_4px_12px_rgba(37,99,235,0.3)] disabled:shadow-none active:scale-95"
            >
              {sending ? (
