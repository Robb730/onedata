...
function getFileIcon(type) {
  switch (type) {
    case "PDF":   return { Icon: FileType,       color: "text-red-500",    bg: "bg-red-50"    };
    case "Excel": return { Icon: FileSpreadsheet, color: "text-green-600",  bg: "bg-green-50"  };
    case "Spreadsheet": return { Icon: FileSpreadsheet, color: "text-green-600",  bg: "bg-green-50"  };
    ...
    default:      return { Icon: File,            color: "text-gray-400",   bg: "bg-gray-50"   };
  }
}
...
export default function FolderDetailPage() {
  ...
  const typeCounts = FILE_TYPE_TABS.reduce((acc, t) => {
    acc[t] = t === "All" ? allFiles.length : allFiles.filter((f) => f.type === t).length;
    return acc;
  }, {});
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-2 text-sm text-slate-400 mb-4">
        ...
        <span>/</span>
        <span className="text-slate-700 font-semibold truncate max-w-100">{decodedName}</span>
      </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {allFiles.length} files · Last modified {allFiles[0]?.date ?? "—"}
              </p>
            </div>
          </div>
        </div>
      ...
      <p className="text-sm text-gray-500 mb-4">
        Showing <span className="font-semibold text-gray-700">{filtered.length}</span> of {allFiles.length} files
      </p>
            ...
            {filtered.map((file) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Type */}
                  <span className={`inline-flex w-fit items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border ${getFileIcon(file.type).color} bg-opacity-10`}
                    style={{ backgroundColor: undefined }}
                  >
                    <span className={`text-[11px] font-bold ${getFileIcon(file.type).color}`}>{file.type}</span>
                  </span>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs text-gray-600 truncate">{file.uploader}</span>
                  </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${getStatusStyle(file.status)}`}>
                    {file.status}
                  </span>
